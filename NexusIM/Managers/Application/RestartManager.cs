...
namespace InstantMessage
{
	class RestartManager
	{
		...
		private static void BeginRestart()
		{
			...
			{
				XmlElement elem = xml.CreateElement("contactlist");
				windows.AppendChild(elem);
			}

			Trace.WriteLine("Saving " + ProtocolManager.Instance.Windows.Count + " window(s)");

			foreach (frmChatWindow win in ProtocolManager.Instance.Windows)
			{
				XmlElement elem = xml.CreateElement("window");
				XmlElement elem2 = xml.CreateElement("buddyname");
				elem.SetAttribute("type", "chat");
				elem2.Value = win.Buddy.Username;
