...
namespace NexusIM
{
	...
	static class TimezoneFuncs
	{
		public static string ConvertToLocal(string input)
		{
			...
			// Here be dragons
			MatchCollection detectedtimes = Regex.Matches(input, @"([0-9]{1,2}:[0-9]{2})(?: ([AP]M))? (?:\(((?:GMT)|(?:UTC))([- ][0-9]{1,2}:[0-9]{2})\))?");
			...
			while (timeEnum.MoveNext())
			{
				...
				if (time.Groups[3].Success) // Did the user include the timezone stamp for us - "(GMT-8)"
				{
					DateTime dtime = DateTime.Parse(time.Groups[1].Value.ToString());
					dtime = dtime.AddHours(-(Convert.ToInt32(time.Groups[4].Value.ToString()) - TimeZoneInfo.Local.BaseUtcOffset.Hours));
					if (time.Groups[5].Success)
						dtime = dtime.AddMinutes(-(Convert.ToInt32(time.Groups[5].Value.ToString()) - TimeZoneInfo.Local.BaseUtcOffset.Minutes));
